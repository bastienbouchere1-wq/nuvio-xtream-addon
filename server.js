...
async function getThemeIndex(config, opts) {
  const key = `themeidx:${config.host}:${config.user}:${opts.cleanNames}:${opts.hideJunk}:${opts.logos}:${opts.hideAdult}:${(opts.langs || []).join(',')}`;
  ...
  (cats || []).forEach(c => {
    ...
    const nm = cleanName(c.category_name);
    catNameById[id] = nm;
    const adultCat = opts.hideAdult && isAdultCategory(c.category_name);
    const allowed = lang.categoryAllowed(nm, opts.langs);
    if (adultCat || !allowed) blocked.add(id);
  });
  const filtered = (streams || []).filter(st => !blocked.has(String(st.category_id)));
...
app.get('/:config/meta/:type/:id.json', withConfig, async (req, res) => {
  try {
    ...
    if (parsed.kind === 'lg') {
      ...
      const opts = getOpts(config);
      const base = baseUrl(req);
      // Logo (HD via proxy, sinon logo Xtream)
      let poster;
      const slug = opts.logos ? logos.resolveLogoSlug(obj.n) : null;
      if (opts.logos && slug) { const fb = obj.i ? Buffer.from(obj.i, 'utf8').toString('base64url') : '_'; poster = `${base}/l/${slug}/${fb}.png`; }
      else poster = obj.i || undefined;
      // Programme en cours + à suivre (EPG)
      let description;
      try {
        const sid = obj.v && obj.v[0] && obj.v[0][0];
        if (sid) {
          const epg = await xtream.getShortEpg(config, sid, 3);
          const list = (epg && (epg.epg_listings || epg.epg_listing)) || [];
          ...
          if (list.length) {
            ...
            if (nx) lines.push('\n⏭ À suivre : ' + (dec(nx.title) || '') + (nx.start_timestamp ? ` (${hm(nx.start_timestamp)})` : ''));
