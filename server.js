    ...
    if (opts.types.live) {
      if (opts.liveOrg === 'theme') {
        ...
        themes.order.forEach(k => {
          ...
          const extra = opsg.length > 1 ? [{ name: 'genre', options: opsg }, { name: 'skip' }] : [{ name: 'skip' }];
          catalogs.push({ type: 'tv', id: 'xtream-theme-' + k, name: themes.labels[k], extra });
        });
      } else {
...
app.get('/:config/catalog/:type/:rest(*)', withConfig, async (req, res) => {
  try {
    ...
    if (catalogId.startsWith('xtream-theme-')) {
      ...
      if (extra.genre) list = list.filter(g => (g.cats || []).includes(extra.genre));
