...
// Construit les 'extra' d'un catalogue (affichage normal + genre optionnel si dispo).
function catExtra(genreOptions) {
  const go = (genreOptions && genreOptions.length) ? genreOptions : [];
  return go.length > 1 ? [{ name: 'genre', options: go }, { name: 'skip' }] : [{ name: 'skip' }];
}

// Hash court pour rendre l'identifiant du manifest unique par configuration (anti-cache Nuvio)
  ...
  try {
    ...
    if (opts.types.live) {
      if (opts.liveOrg === 'theme') {
        ...
        themes.order.forEach(k => {
          ...
          catalogs.push({ type: 'tv', id: 'xtream-theme-' + k, name: themes.labels[k], extra: catExtra(opsg) });
        });
      } else {
        ...
        (liveCats || []).forEach(c => {
          ...
          catalogs.push({ type: 'tv', id: 'xtream-live-' + c.category_id, name: cleanName(c.category_name), extra: catExtra() });
        });
      }
    }
    ...
    const addCats = async (getter, type, prefix, selType) => {
      ...
      (cats || []).forEach(c => {
        ...
        catalogs.push({ type, id: `${prefix}-${c.category_id}`, name: cleanName(c.category_name), extra: catExtra() });
      });
    };
    ...
    res.json({
      id: 'community.xtream.nuvio.' + shortHash(JSON.stringify({ h: config.host, u: config.user, o: config.opts || {}, c: config.cats || {} })),
      version: '1.7.1',
      name: 'IPTV Xtream',
