...
function isJunkName(raw) {
  const s = (raw || '').trim();
  if (!s) return true;
  const stripped = s.replace(/^\s*[A-Za-z]{2,3}\s*[-|:]\s*/, '').trim(); // sans préfixe "FR - "
  if (!stripped || /^[\W_]+$/.test(stripped)) return true;  // vide / que des symboles
  if (/[#=~\-–—_.*•]{3,}/.test(s)) return true;             // séparateur provider
  if (/^\(?\s*flsp\b/i.test(stripped)) return true;         // flux d'événements FloSports
  if (/no event|no stream|no signal|coming soon/i.test(s)) return true; // slots PPV vides
  if (/^\d{1,3}$/.test(stripped)) return true;              // noms "0", "12"
  if (/\bradio\b/i.test(s)) return true;                    // stations de radio
  return false;
}
...
function liveGroupId(g) {
  const icon = (g.xtreamIcon && g.xtreamIcon.length <= 160) ? g.xtreamIcon : '';
  return `${PREFIX}:lg:${b64urlEncode({ n: g.name, i: icon, v: g.variants.map(v => [v.id, v.r, v.label]) })}`;
}
...
function buildLiveThemes(streams, opts, catNameById) {
  ...
  for (const k in buckets) buckets[k].sort((a, b) => {
    const ra = themes.rank(a.name), rb = themes.rank(b.name);
